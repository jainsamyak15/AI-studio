...
//     const keywords = [
//       'iconic', 'timeless', 'scalable', 'versatile', 'memorable',
//       'simple', 'bold', 'unique', 'professional', 'relevant',
//       'balanced', 'symmetrical', 'asymmetrical', 'geometric', 'organic',
//       'monochrome', 'colorful', 'gradient', 'flat design', 'minimalist'
//     ];
//     const randomKeywords = getRandomKeywords(keywords, 4);
//     return `Design a logo: ${prompt}. Style: ${randomKeywords.join(', ')}. Create a professional, versatile logo suitable for various applications. Ensure it's simple yet memorable, and avoid overly complex designs. The logo should work well in both color and monochrome, and be scalable for different sizes.`;
//   };
// src/utils/enhancePrompt.ts
...
const enhanceLogoPrompt = (prompt: string): string => {
  ...
  const enhanceTaglinePrompt = (prompt: string): string => {
    const keywords = [
      ...
      'authentic', 'clear', 'aspirational', 'timeless', 'relevant'
    ];
    const randomKeywords = getRandomKeywords(keywords, 3);
    return `Generate a tagline for: ${prompt}. Style: ${randomKeywords.join(', ')}. Create a short, memorable phrase that captures the essence of the brand or product. The tagline should be no more than 5-7 words, easy to understand, and emotionally resonant. Avoid clichés and overly complex language.Do not include any numbering, explanations, or additional text. Just provide the taglines, each on a new line.`;
  };
